...
run_demo() {
    python3 "$SCRIPT_DIR/ascii_demo.py" $MODE $ANIM
}
...
run_all_scenarios() {
    ...
    python3 "$SCRIPT_DIR/ascii_demo.py" --mock success $ANIM
    echo -e "\n${GRAY}Press Enter for next scenario...${RESET}"
    read -r
    
    echo -e "${YELLOW}━━━ MIXED SCENARIO ━━━${RESET}"
    python3 "$SCRIPT_DIR/ascii_demo.py" --mock mixed $ANIM
    echo -e "\n${GRAY}Press Enter for next scenario...${RESET}"
    read -r
    
    echo -e "${RED}━━━ FAILURE SCENARIO ━━━${RESET}"
    python3 "$SCRIPT_DIR/ascii_demo.py" --mock failure $ANIM
...
if [ "$ALL_SCENARIOS" = true ]; then
    ...
    echo -e "${GRAY}Watching ascii_demo.py for changes...${RESET}"
    echo ""
    ...
    if command -v inotifywait &> /dev/null; then
        while true; do
            ...
            inotifywait -q -e modify "$SCRIPT_DIR/ascii_demo.py" 2>/dev/null || sleep 2
            clear
        done
    else
